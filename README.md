# Hi, I'm Changjoon Baek 👋

I build developer tools around **testing, code quality, and LLM coding agents**.

## 🔧 Projects

| Project | What it does |
|---|---|
| [atlassian-browser-skills](https://github.com/baekchangjoon/atlassian-browser-skills) | Read/write Jira & Confluence from an LLM agent via your logged-in local browser — no API token, no MCP |
| [hotspot-analysis](https://github.com/baekchangjoon/hotspot-analysis) | Rank Java code by a deterministic Composite Hotspot Score (churn × complexity × coverage gap) to prioritize test generation |
| [encrypted-memory](https://github.com/baekchangjoon/encrypted-memory) | Encrypt LLM agent memory files at rest (age-based); decrypt only in unlocked sessions |
| [ManualTestCoverageMeasurement](https://github.com/baekchangjoon/ManualTestCoverageMeasurement) | Measure backend (JaCoCo) + frontend (V8/CDP) coverage during manual testing via a Chrome extension |
| [CEF-Browser](https://github.com/baekchangjoon/CEF-Browser) | Custom Windows browser on CefSharp, with a [Playwright E2E suite](https://github.com/baekchangjoon/CEF-Browser-Playwright) |

## 🛠 Tech

Java/Spring Boot · TypeScript · Go · C#/.NET · Playwright · JaCoCo · Claude Code / agent skills
